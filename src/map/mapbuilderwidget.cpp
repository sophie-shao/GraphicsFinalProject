...
void MapBuilderWidget::onMapHeightChanged(int value)
{
    generateBiomeMap();
    // Height map will be regenerated in generateBiomeMap()
    // Map will be updated in generateBiomeMap()
}
...
void MapBuilderWidget::saveMapScene()
{
    ...
    // Calculate camera position to view the entire map
    // Center the map at origin, scale appropriately
    float mapCenterX = mapWidth * 0.5f;
    float mapCenterZ = mapHeight * 0.5f;
    float maxDimension = std::max(mapWidth, mapHeight);
    
    // Find the middle block's position and height for camera positioning
    ...
    // Calculate middle block world position (centered at origin)
...
{
    // Initialize the Map from the MapBuilder
