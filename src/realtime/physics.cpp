...
void Physics::updatePhysics(Realtime* realtime, float deltaTime) {
    ...
    const float BASE_MOVE_SPEED = 25.0f;
    ...
    if (realtime->m_keyMap[Qt::Key_W]) {
        realtime->m_velocity += forward * moveSpeed * deltaTime;
    }
    if (realtime->m_keyMap[Qt::Key_S]) {
        realtime->m_velocity -= forward * moveSpeed * deltaTime;
    }
    if (realtime->m_keyMap[Qt::Key_A]) {
        realtime->m_velocity -= right * moveSpeed * deltaTime;
    }
    if (realtime->m_keyMap[Qt::Key_D]) {
        realtime->m_velocity += right * moveSpeed * deltaTime;
    }
    
    if (realtime->m_keyMap[Qt::Key_Space] && realtime->m_onGround) {
    ...
    for (int step = 0; step < subSteps; step++) {
        ...
        float friction = realtime->m_onGround ? GROUND_FRICTION : AIR_FRICTION;
        realtime->m_velocity.x *= friction;
        realtime->m_velocity.z *= friction;
